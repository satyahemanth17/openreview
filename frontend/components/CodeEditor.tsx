...
interface CodeEditorProps {
  ...
  onAddLineComment?: (lineStart: number, lineEnd: number, body: string, pane: 'original' | 'modified') => Promise<void>;
  onEditorReady?: () => void;
}

const CodeEditor = forwardRef<CodeEditorHandle, CodeEditorProps>(
  function CodeEditor({ filename, patch, onLineClick, onAddLineComment, onEditorReady }, ref) {
    ...
    async function handleSubmit() {
      ...
      setSubmitting(true);
      try {
        await onAddLineComment(overlay.lineStart, overlay.lineEnd, inputBody.trim(), overlay.pane);
        setInputBody('');
        setShowInput(false);
        setOverlay(null);
      } finally {
